...
# cleav_suffix = "WT"
# c
# data_add = "/data/zenglin/goldenham/degradome/wkdir/PIWIL3_PIWIL3_associated_WT_MUT/"
# num_of_rep_cleav = 2
# num_of_rep_nocleav = 2
# set_range = 20
# risc = "PIWIL3_PIWIL3_associated"
# targetseq_len = "/data/zenglin/goldenham/degradome/transcriptome/BCM_Macur_seq_stat.txt"
...
process_cleav <- function(cleav_suffix_rep) {
  ...
  flagstat1 <- readLines(paste0(cleav_suffix_rep, "_R1.trimmed.flagstat.txt"))
  ...
  cleav$strand <- ifelse(cleav$strand == 0, "+", "-") # FIXME the reads are unique here without duplication
  range2ext <- set_range
  #if(reference=="1"){
  ...
  write.table(
    file = paste0(
      data_add,
      unlist(strsplit(cleav_suffix_rep, "_"))[1],
      "_3CleavProductsStatNorm.txt"
    ),
    reduced_cleav,
    sep = "\t",
    quote = FALSE,
...
events <- merge(events, total_table, by = "uniqpos") # total_table has abudnace info events has basic info
events_uniq <- events
...
output_file <- paste0(data_add, risc, "_insert_expr.txt")
write.table(
  file = output_file,
  events_uniq,
  sep = "\t",
...
output_file <- paste0(data_add, risc, "_insert.bed")
